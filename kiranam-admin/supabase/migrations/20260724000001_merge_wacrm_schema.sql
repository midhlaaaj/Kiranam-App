-- ============================================================
-- Merge wacrm's schema into Kiranam's Supabase project.
--
-- This is a HAND-CURATED derivative of wacrm's own migrations
-- (github.com/ArnasDon/wacrm, supabase/migrations/001-036, see
-- kiranam-admin/wacrm-setup/combined_schema.sql for the untouched
-- original). It is NOT a verbatim replay — 8 sections were removed
-- because they assume wacrm owns 'profiles' outright (single-tenant
-- -> personal-account backfill, RLS rewrite, signup trigger), which
-- would have mass-created a wacrm workspace for every contributor
-- and volunteer, and would have weakened Kiranam's own profiles RLS
-- (specifically the self-promotion-to-admin protection). See the
-- planning conversation for the exact line-range reasoning.
--
-- BACKUP KIRANAM'S SUPABASE PROJECT BEFORE RUNNING THIS. Real donor
-- and contribution data lives here.
--
-- Idempotent throughout (inherited from wacrm's own convention, plus
-- the Kiranam-specific additions below follow the same IF NOT EXISTS
-- / DROP-then-CREATE pattern) — safe to re-run if something fails
-- partway and you retry.
-- ============================================================

-- ============================================================
-- PART 1 — Prepare Kiranam's existing profiles table
--
-- Additive only. Kiranam's profiles.id already IS auth.users.id
-- (unlike wacrm's own profiles, which uses a surrogate id + separate
-- user_id). Backfilling user_id = id gives wacrm's RLS/RPCs (which
-- only ever reference profiles.user_id, confirmed by reading all 36
-- migrations) a working, always-correct identity key with zero
-- changes to wacrm's own business logic.
--
-- account_id / account_role are added by the wacrm tables section
-- below (they're part of migration 017) — deliberately left NULLable
-- there and never forced NOT NULL, so a donor/volunteer profile with
-- no comm-center access is completely normal, not an error state.
-- ============================================================
...
-- ============================================================
-- PART 2 — wacrm's own tables, verbatim except:
--   * profiles CREATE TABLE + RLS + handle_new_user: removed (Kiranam's
--     profiles already exists; Part 1 above + Part 4 below replace these)
--   * the mass personal-account backfill DO block (017): removed
--   * profiles account_id/account_role forced NOT NULL (017): removed,
--     stay nullable
--   * the RLS-rewrite DROP-policy loop's 'profiles' target (017):
--     removed from the array — every OTHER table in that loop still
--     has its own explicit DROP POLICY IF EXISTS right before its
--     CREATE POLICY, so removing the loop entirely (not just editing
--     the array) has no effect on any table except profiles
--   * "RLS — PROFILES (revised)" block (017): removed — would have
--     added a permissive UPDATE policy with no role-change protection,
--     running alongside (OR'd with, i.e. weakening) Kiranam's existing
--     self-promotion-blocking policy from profiles_role_lockdown.sql
--   * signup trigger replacement (017): removed — Part 4 below is the
--     single authoritative handle_new_user
--   * notifications table (027): renamed to wacrm_notifications
--     throughout — collides by name with Kiranam's own notifications
--     table (donor-facing announcements, unrelated purpose)
-- ============================================================

-- ============================================================
-- wacrm (github.com/ArnasDon/wacrm) — combined schema migration
-- Concatenation of supabase/migrations/001..036, in order, exactly
-- as fetched from the upstream repo (main branch).
-- Paste this whole file into a NEW Supabase project's SQL Editor
-- and run it once. This is wacrm's OWN schema — do not run it
-- against Kiranam's existing Supabase project (it creates a
-- conflicting public.profiles table).
-- ============================================================
...
-- ======================================================
-- ======================================================
-- 027_notifications.sql (renamed: notifications -> wacrm_notifications)
--
-- wacrm's own table name collides with Kiranam's existing
-- public.notifications (donor-facing announcements, unrelated
-- purpose: category contribution|campaign|system). Renamed at
-- creation so both coexist without touching Kiranam's table.
-- ======================================================
-- ============================================================
-- WACRM_NOTIFICATIONS
-- ============================================================
...
DROP POLICY IF EXISTS wacrm_notifications_select ON wacrm_notifications;
DROP POLICY IF EXISTS wacrm_notifications_update ON wacrm_notifications;
CREATE POLICY wacrm_notifications_select ON wacrm_notifications FOR SELECT
  USING (auth.uid() = user_id);
CREATE POLICY wacrm_notifications_update ON wacrm_notifications FOR UPDATE
  USING (auth.uid() = user_id)
...
BEGIN
  ...
  LOOP
    ...
    UPDATE wacrm_notifications SET conversation_id = v_survivor WHERE conversation_id = ANY(v_losers);
...
-- ============================================================
-- PART 3 — Sync linkage + offline-payment columns
-- ============================================================
...
-- ============================================================
-- PART 4 — Merged handle_new_user()
--
-- Single authoritative version. Keeps Kiranam's original behavior
-- (profiles row keyed by id = auth.users.id, phone/email from the
-- new auth user, role defaults to 'contributor' per the column's own
-- DEFAULT) and additionally sets user_id = id. Deliberately does NOT
-- auto-create a wacrm account/account_role — comm-center access is
-- provisioned explicitly (Part 5), not granted by the act of signing
-- up, so a contributor or volunteer signing up in the mobile app
-- never gets a wacrm workspace.
-- ============================================================
...
-- ============================================================
-- PART 5 — Auto-provision wacrm access for Kiranam admins
--
-- One shared "Kiranam" wacrm account/workspace for every admin, at
-- account_role = 'admin' (full comm-center access; not 'owner' —
-- no destructive account-level actions like deleting the whole
-- workspace). Fires whenever a profile's role becomes 'admin',
-- whether by INSERT or UPDATE.
-- ============================================================

CREATE OR REPLACE FUNCTION public.provision_wacrm_admin_access()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Failed to provision wacrm access for admin %: %', NEW.id, SQLERRM;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS on_profile_admin_provision_wacrm ON public.profiles;
CREATE TRIGGER on_profile_admin_provision_wacrm
  AFTER INSERT OR UPDATE OF role ON public.profiles
  FOR EACH ROW
  WHEN (NEW.role = 'admin')
  EXECUTE FUNCTION public.provision_wacrm_admin_access();

-- ============================================================
-- PART 6 — Sync contributors/volunteers into wacrm as contacts
--
-- Auto-tags by role ("Contributor" / "Volunteer") on the shared
-- Kiranam account so they're immediately targetable by wacrm's
-- existing tag-based broadcast audience selector — no new UI needed.
-- No-ops (returns early) until at least one admin has been
-- provisioned (Part 5) and the shared account exists.
-- ============================================================
...
BEGIN
  ...
  VALUES (v_account_id, NEW.id, NEW.phone, COALESCE(NULLIF(NEW.full_name, ''), NEW.phone), NEW.id)
...
-- ============================================================
-- PART 7 — One-time backfill for rows that existed before these
-- triggers did. Order matters: admins first (creates the shared
-- account), then contributors/volunteers (syncs into it).
--
-- If there are zero existing admins when this runs, the contributor/
-- volunteer backfill below silently no-ops (no account to sync into
-- yet) — re-run both UPDATE statements after promoting your first
-- admin to backfill everyone retroactively.
-- ============================================================

UPDATE public.profiles SET role = role WHERE role = 'admin';
UPDATE public.profiles SET role = role WHERE role IN ('contributor', 'volunteer');
