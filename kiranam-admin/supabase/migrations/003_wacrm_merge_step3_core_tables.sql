-- ============================================================
-- wacrm merge, step 3 of 4 -- remaining ~30 core tables
--
-- Run this AFTER steps 1 and 2. This is wacrm's own migrations
-- 001-036 concatenated in order, with every profiles-touching
-- and notifications-touching statement removed (handled in
-- steps 1/2/4 instead) -- specifically: the original profiles
-- CREATE TABLE + policies, the original handle_new_user()
-- trigger replacement, and the notifications table (renamed
-- to wacrm_notifications in step 4). None of these ~30 tables
-- collide with Kiranam's existing schema by name.
-- ============================================================
...
-- ============================================================
-- FIX: add account_id to every domain table (originally wacrm's
-- migration 017, which this file otherwise excludes wholesale
-- because 017 also contains the profiles-specific backfill/RLS/
-- handle_new_user logic that's handled separately in step 2).
-- All 15 tables exist by this point in the file; every later
-- migration below (018+) that references account_id on them
-- needs it to already be here. Nullable is fine -- these tables
-- are brand new and empty, no backfill needed; later migrations
-- below may still apply their own NOT NULL if they expect it.
-- ============================================================
...
-- flow_runs, wacrm_notifications, ai_usage_log) are re-pointed to the
...
BEGIN
  ...
  LOOP
    ...
    -- message_reactions, wacrm_notifications) from being removed with the
    -- loser conversations.
