...
import {
  ...
  blankButtonsPayload,
  blankListPayload,
} from "@/components/whatsapp/interactive/interactive-builder"
import { interactivePayloadPreviewText } from "@/lib/whatsapp/whatsapp/interactive"
...
// "create_deal" is deliberately excluded — Pipelines/Deals was removed
// as a feature (no NGO fit), see wacrm/src/app/(dashboard)/pipelines/page.tsx.
// STEP_META above still carries an entry for it (TypeScript requires
// every AutomationStepType to have one) and the engine/validator still
// handle it — this only hides it from the "add step" picker.
...
function ResourcesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/whatsapp/account/members", { cache: "no-store" })
        if (!res.ok) return
...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = isEditing
        ? await fetch(`/api/whatsapp/automations/${initial.id}`, {
            method: "PATCH",
      ...
      <div className="rounded-lg border border-border border-l-4 border-l-primary bg-card shadow-lg">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] uppercase tracking-wide text-primary">{t("trigger")}</div>
