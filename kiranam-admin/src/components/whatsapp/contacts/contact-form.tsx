...
} from '@/lib/whatsapp/contacts/dedupe';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
// Common WhatsApp market dial codes. `phone` is stored/sent as
// `${countryCode}${localNumber}` (E.164, no separators) — splitting the
// input avoids users omitting or misplacing the country code.
const COUNTRY_CODES = [
  { code: '+91', label: 'India' },
...
/** Split a stored E.164 phone into a known country code + local number. */
function splitPhone(phone: string): { countryCode: string; localNumber: string } {
  ...
  return { countryCode: COUNTRY_CODES[0].code, localNumber: value.replace(/^\+/, '') };
}
...
export function ContactForm({
  ...
  const phone = localNumber.trim() ? `${countryCode}${localNumber.trim()}` : '';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              >
                <SelectTrigger className="w-28 shrink-0 bg-muted border-border text-foreground">
                  <SelectValue />
                </SelectTrigger>
