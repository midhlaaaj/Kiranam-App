...
export function AiKnowledgeCard({
  ...
  const fetchDocs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/whatsapp/ai/knowledge');
  ...
  const openEdit = async (id: string) => {
    try {
      const res = await fetch(`/api/whatsapp/ai/knowledge/${id}`);
      const data = await res.json();
      if (!res.ok) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(
        isNew ? '/api/whatsapp/ai/knowledge' : `/api/whatsapp/ai/knowledge/${editing}`,
        {
  ...
  const reindex = async () => {
    setReindexing(true);
    try {
      const res = await fetch('/api/whatsapp/ai/knowledge/reindex', { method: 'POST' });
      const data = await res.json();
