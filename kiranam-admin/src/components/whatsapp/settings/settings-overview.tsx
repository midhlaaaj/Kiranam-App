    ...
    (async () => {
      ...
      const [membersRes, invitesRes, tagsRes, fieldsRes] =
        await Promise.allSettled([
          fetch('/api/whatsapp/account/members', { cache: 'no-store' }).then((r) => r.json()),
          canManageMembers
            ? fetch('/api/whatsapp/account/invitations', { cache: 'no-store' }).then((r) =>
                r.json(),
              )
    ...
    (async () => {
      ...
      const [row, health] = await Promise.allSettled([
        supabase
          ...
          .maybeSingle(),
        fetch('/api/whatsapp/whatsapp/config', { cache: 'no-store' }).then((r) => r.json()),
      ]);
      if (cancelled) return;
      setWhatsapp({
