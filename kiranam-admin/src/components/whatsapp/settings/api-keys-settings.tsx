...
// (`/api/whatsapp/v1/*`). Any member sees the roster (read-only); admin+ can
...
import {
  ...
  SCOPE_DESCRIPTIONS,
  type ApiScope,
} from '@/lib/whatsapp/api-keys/scopes';
...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch('/api/whatsapp/account/api-keys', { cache: 'no-store' });
      if (!res.ok) {
  ...
  async function handleRevoke(key: ApiKey) {
    setRevoking(key.id);
    try {
      const res = await fetch(`/api/whatsapp/account/api-keys/${key.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/whatsapp/account/api-keys', {
        method: 'POST',
