...
import { RequireRole } from '@/components/whatsapp/auth/require-role';
import { useAuth } from '@/hooks/whatsapp/use-auth';
import { usePresence } from '@/hooks/whatsapp/use-presence';
import type { AccountRole } from '@/lib/whatsapp/auth/roles';
import { presenceLabel, summarize } from '@/lib/whatsapp/presence';
import {
  PRESENCE_DOT_CLASS,
  PresenceDot,
} from '@/components/whatsapp/presence/presence-dot';
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      const [mres, ires] = await Promise.all([
        fetch('/api/whatsapp/account/members', { cache: 'no-store' }),
        canManageMembers
          ? fetch('/api/whatsapp/account/invitations', { cache: 'no-store' })
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      const res = await fetch(`/api/whatsapp/account/members/${member.user_id}`, {
        method: 'PATCH',
  ...
  async function handleRemove() {
    ...
    try {
      const res = await fetch(
        `/api/whatsapp/account/members/${removingMember.user_id}`,
        { method: 'DELETE' },
      );
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
